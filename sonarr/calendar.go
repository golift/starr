...
func (s *Sonarr) GetCalendarContext(ctx context.Context, filter Calendar) ([]*Episode, error) {
	...
	req.Query.Add("includeSeries", starr.Itoa(filter.IncludeSeries))
	req.Query.Add("includeEpisodeFile", starr.Itoa(filter.IncludeEpisodeFile))
	req.Query.Add("includeEpisodeImages", starr.Itoa(filter.IncludeEpisodeImages))
