...
func TestAddDownloadClient(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(test.Name, func(t *testing.T) {
			...
			mockServer := test.GetMockServer(t)
			client := sonarr.New(starr.New("mockAPIkey", mockServer.URL, 0))
			output, err := client.AddDownloadClient(test.WithRequest.(*sonarr.DownloadClientInput))
			require.ErrorIs(t, err, test.WithError, "error is not the same as expected")
			assert.EqualValues(t, test.WithResponse, output, "response is not the same as expected")
		})
	}
...
func TestUpdateDownloadClient(t *testing.T) {
	...
	tests := []*starrtest.MockData{
		{
			Name:           "200",
			ExpectedPath:   path.Join("/", starr.API, sonarr.APIver, "downloadClient", "3?forceSave=false"),
			ExpectedMethod: "PUT",
			ResponseStatus: 200,
			WithRequest: &sonarr.DownloadClientInput{
				Enable:                   true,
	...
	for _, test := range tests {
		...
		t.Run(test.Name, func(t *testing.T) {
			...
			mockServer := test.GetMockServer(t)
			client := sonarr.New(starr.New("mockAPIkey", mockServer.URL, 0))
			output, err := client.UpdateDownloadClient(test.WithRequest.(*sonarr.DownloadClientInput), false)
			require.ErrorIs(t, err, test.WithError, "error is not the same as expected")
			assert.EqualValues(t, test.WithResponse, output, "response is not the same as expected")
		})
	}
