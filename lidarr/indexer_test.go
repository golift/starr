...
const indexerResponseBody = `{
	...
	"infoLink": "https://wiki.servarr.com/lidarr/supported#newznab",
	"tags": [],
	"id": 1
  }`
	...
	tests := []*starrtest.MockData{
		{
			...
			WithRequest:     nil,
			WithResponse: []*lidarr.IndexerOutput{
				{
					EnableAutomaticSearch:   true,
					EnableInteractiveSearch: true,
					EnableRss:               true,
		...
		},
		{
			...
			WithResponse:   ([]*lidarr.IndexerOutput)(nil),
		},
	}
...
func TestUpdateIndexer(t *testing.T) {
	...
	tests := []*starrtest.MockData{
		{
			Name:           "200",
			ExpectedPath:   path.Join("/", starr.API, lidarr.APIver, "indexer", "1?forceSave=false"),
			ExpectedMethod: "PUT",
			ResponseStatus: 200,
			WithRequest: &lidarr.IndexerInput{
				EnableAutomaticSearch:   true,
