...
func (r *Radarr) GetMovieContext(ctx context.Context, getMovie *GetMovie) ([]*Movie, error) {
	...
	if getMovie.TMDBID != 0 {
		params.Set("tmdbId", starr.Str(getMovie.TMDBID))
	} else {
		// excludeLocalCovers can only be true without a tmdbid.
		params.Set("excludeLocalCovers", starr.Str(getMovie.ExcludeLocalCovers))
	}
...
func (r *Radarr) GetMovieByIDContext(ctx context.Context, movieID int64) (*Movie, error) {
	...
	req := starr.Request{URI: path.Join(bpMovie, starr.Str(movieID))}
	if err := r.GetInto(ctx, req, &output); err != nil {
		return nil, fmt.Errorf("api.Get(%s): %w", &req, err)
	}
...
func (r *Radarr) UpdateMovieContext(ctx context.Context, movieID int64, movie *Movie, moveFiles bool) (*Movie, error) {
	...
	req := starr.Request{
		URI:   path.Join(bpMovie, starr.Str(movieID)),
...
func (r *Radarr) LookupIDContext(ctx context.Context, movieID int64) (*Movie, error) {
	return r.lookupSubContext(ctx, starr.Str(movieID), "", "")
}
...
func (r *Radarr) LookupTMDBContext(ctx context.Context, tmdbID int64) (*Movie, error) {
	return r.lookupSubContext(ctx, "tmdb", "tmdbId", starr.Str(tmdbID))
}
...
func (r *Radarr) DeleteMovieContext(ctx context.Context, movieID int64, deleteFiles, addImportExclusion bool) error {
	req := starr.Request{URI: path.Join(bpMovie, starr.Str(movieID)), Query: make(url.Values)}
	req.Query.Set("deleteFiles", starr.Str(deleteFiles))
	req.Query.Set("addImportExclusion", starr.Str(addImportExclusion))
